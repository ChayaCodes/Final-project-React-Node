...
import {useAddForumMutation} from "../forumApiSlice";
...
const AddForum = () => {
  ...
  const [addForum, { data, isError, error, isLoading, isSuccess }] = useAddForumMutation();
  ...
  return (
    <div className='add-forum-container'>
      <form className='add-forum-form'>
        ...
        <button className='add-forum-submit' onClick={handleSubmit}>{isLoading && <CircularProgress size={20} color='inherit' />}הוסף פורום </button>
      </form>
